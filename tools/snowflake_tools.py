...
class SnowflakeTool(BaseTool):
    ...
    async def validate(self) -> ToolResult:
        ...
        try:
            ...
            self._cursor.execute(
                "SELECT CURRENT_WAREHOUSE() AS WAREHOUSE,CURRENT_DATABASE() AS DATABASE,CURRENT_SCHEMA() AS SCHEMA,CURRENT_ROLE() AS ROLE,CURRENT_USER() AS USER;"
            )
            ...
            return ToolResult(
                ...
                execution_time_ms=execution_time,
                metadata={
                    "query": "SELECT CURRENT_WAREHOUSE() AS WAREHOUSE,CURRENT_DATABASE() AS DATABASE,CURRENT_SCHEMA() AS SCHEMA,CURRENT_ROLE() AS ROLE,CURRENT_USER() AS USER"
                },
            )
    ...
    async def execute(self, query: str, **kwargs) -> ToolResult:
        ...
        try:
            ...
            if self._cursor.description:
                ...
                rows_affected = (
                    self._cursor.rowcount if self._cursor.rowcount >= 0 else 0
                )
                execution_time = (time.time() - start_time) * 1000
                return ToolResult(
                    tool_name=self.name,
    ...
    def format_results_table(
        self, results: List[Dict[str, Any]], max_rows: int = 20
    ) -> str:
        """
        ...
        try:
            ...
            table = tabulate(
                rows, headers=headers, tablefmt=self.settings.ui.table_format
            )
