    ...
    async def _execute_tool(self, tool_name: str, arguments: Dict[str, Any]) -> ToolResult:
        ...
        try:
            if tool_name == "execute_snowflake_query":
                ...
                result = await self.github.execute(operation="list_directory", **arguments)
                ...
                result = await self.github.execute(operation="create_or_update_file", **arguments)
                
            elif tool_name == "trigger_github_workflow":
                result = await self.github.execute(operation="trigger_workflow", **arguments)
                
            elif tool_name == "list_github_workflow_runs":
                # Map limit to per_page
                if "limit" in arguments:
                    arguments["per_page"] = arguments.pop("limit")
                result = await self.github.execute(operation="list_workflow_runs", **arguments)
                
            elif tool_name == "get_github_workflow_run":
                result = await self.github.execute(operation="get_workflow_run", **arguments)
                
            elif tool_name == "get_github_workflow_run_jobs":
                result = await self.github.execute(operation="get_workflow_run_jobs", **arguments)
