# Phase 2 deferred post-conditions

During Phase 1 (the Great Decoupling), the generic `Dispatcher` replaced the
...
They have been **removed** in Phase 1 and must be **reimplemented as proper
post-conditions / catalog updates in Phase 2** (where they belong, driven by
...
1. **Created-schema tracking**
   ...
   - Phase 2: detect schema creation from the executed statement / Snowflake
...
2. **Created file-format tracking**
   ...
   - Phase 2: record created file formats from structured execution results.
...
3. **Created-table tracking**
   ...
   - Phase 2: record created tables from structured execution results.

## Memory API still available for the Phase 2 reimplementation
...
- **Application moved into the dispatcher** (`Dispatcher._apply_catalog_effects`),
  ...
  (this is where Phase 1's risk metadata first earns its keep — write-invalidation).
