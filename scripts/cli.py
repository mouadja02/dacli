...
@click.option(
    ...
    "-p",
    type=str,
    help="Quick profile: full, github_only, snowflake_only, datawarehouse",
)
def setup(config, profile):
    ...
    if profile:
        ...
        if tools_settings:
            ...
            console.print(
                "Available profiles: full, github_only, snowflake_only, datawarehouse"
            )
    else:
        # Run full wizard
    ...
    with console.status("[bold green]Testing Pinecone connection..."):
        try:
            ...
            if result.success:
                ...
                console.print(
                    f"   Index: {result.data.get('index_name')}, Vectors: {result.data.get('total_vectors')}"
                )
            else:
...
async def _run_chat(
    ...
    if force_setup or not tools_settings.setup_completed:
        ...
        if not tools_settings.setup_completed:
            ...
            run_wizard = Confirm.ask(
                "Would you like to configure which tools to use?", default=True
            )
        else:
            run_wizard = True
    ...
    try:
        if not await agent.initialize():
            console.print(
                "[error]Failed to initialize agent. Check your configuration.[/error]"
            )
            return
    except Exception as e:
        console.print(f"[error]Initialization error: {e}[/error]")
        console.print(
            "[dim]Check your config.yaml and ensure credentials are set.[/dim]"
        )
        return
    ...
    try:
        while True:
            try:
                # Get user input
                user_input = await asyncio.to_thread(pt_session.prompt, "You > ")
                ...
                if user_input.startswith("/"):
                    ...
                    if cmd == "/exit" or cmd == "/quit":
                        ...
                        for msg in memory.get_full_history()[-20:]:
                            ...
                            console.print(
                                f"[{role_style}]{msg.role.upper()}:[/{role_style}] {msg.content[:200]}..."
                            )
                        ...
                        if memory.load_session(args[0]):
                            console.print(
                                f"[success]Loaded session: {args[0]}[/success]"
                            )
                        else:
                            console.print(
                                f"[error]Session not found: {args[0]}[/error]"
                            )
                        ...
                        if Confirm.ask(
                            "Are you sure you want to reset the agent state?"
                        ):
                        ...
                        save_tools_config(tools_settings, config_path or "config.yaml")
                        console.print(
                            "[success]Tools reconfigured. Restart the agent to apply changes.[/success]"
                        )
                ...
                if response.needs_user_input:
                    console.print(
                        "[warning]⏳ Agent is waiting for your input to continue.[/warning]"
                    )
