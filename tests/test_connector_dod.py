...
class AllShippedConnectorsPassDod(unittest.TestCase):
    def test_every_connector_meets_the_dod(self):
        ...
        # After the pivot, no packaged platform connectors exist — only the
        # built-in system/sandbox (exempt from DoD) and dynamically generated
        # extensions. The audit should run without error on an empty set.
        # A non-empty checked list means a connector was discovered and must pass.
        if report.checked:
...
class DodHasTeeth(unittest.TestCase):
    def test_bad_connector_collects_violations(self):
        ...
        violations = check_connector_dod(
            "bad", manifest, _BadConnector(), connector_dir=None
        )
    ...
    def test_compliant_manifest_shape_clears_manifest_rules(self):
        ...
        class _Good(Connector):
            ...
            def operations(self):
                return [
                    OperationSpec(
                        name="introspect_good",
                        description="read",
                        parameters={"type": "object", "properties": {}},
                        capability="good.introspection",
                        risk=Risk.SAFE,
            ...
            async def invoke(self, op, args):
                return ToolResult(
                    tool_name=op, status=ToolStatus.SUCCESS, data={"exists": True}
                )
