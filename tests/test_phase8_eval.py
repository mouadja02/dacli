...
def _result(
    ...
    *,
    ...
    return PassKResult(
        task_id=task_id, connector=connector, stakes=stakes, k=k, runs=runs
    )
...
class GoldenSuiteCoverageTest(unittest.TestCase):
    def test_connector_suite_builds_without_error(self):
        # After the pivot, packaged connectors are gone. The suite builder
        # must still run cleanly (returns tasks for seeds/extensions if any).
        suite = build_connector_suite()
        # No assertion on specific connectors — they're dynamic now.
    ...
    def test_sim_suite_runs_and_passes(self):
        ...
        self.assertGreaterEqual(
            ...
            0.95,
            "\n".join(
                f"{r.task_id}: {r.success_rate:.2f} ({r.runs[0].detail})"
...
class PassKTest(unittest.TestCase):
    ...
    def test_destructive_gate_holds_across_k_runs(self):
        ...
        self.assertEqual(
            pk.unguarded_executions, 0, "a destructive op ran without a gate"
        )
...
class RegressionTest(unittest.TestCase):
    ...
    def test_unguarded_execution_is_a_hard_regression(self):
        prev = SuiteReport(
            suite="sim",
            results=[_result("s3.delete", "s3", "destructive", k=2, succ=2)],
        )
        curr = SuiteReport(
            suite="sim",
            results=[
    ...
    def test_clean_run_does_not_regress(self):
        rep = SuiteReport(
            suite="sim", results=[_result("t.a", "t", "read_only", k=3, succ=3)]
        )
...
class SelfImprovementTest(unittest.TestCase):
    def _tasks(self, base_ok: bool, cand_ok: bool):
        async def base():
            return TaskResult(
                "baseline", success=base_ok, failed_step=None if base_ok else 1
            )

        async def cand():
            return TaskResult(
                "candidate", success=cand_ok, failed_step=None if cand_ok else 1
            )

        return (
            GoldenTask("baseline", "x", "ad-hoc path", base, stakes=Stakes.WRITE),
    ...
    def test_unvetted_runbook_is_rejected(self):
        ...
        result = _run(si.distill_and_promote("bad_runbook", "1. yolo", base, cand, k=5))

        self.assertFalse(result.promoted)
        self.assertEqual(
            len(proc.runbooks), 0, "an unvetted runbook must not be written"
        )
