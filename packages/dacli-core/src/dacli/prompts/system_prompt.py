...
from dacli.core.paths import agents_md_chain, system_md_override, system_md_target
...
# The installed `dacli` package dir. Nothing here is editable for a pip user; a
# user-owned SYSTEM.md (paths.system_md_target) is the writable layer instead.
...
def compose_system_prompt(
    ...
    Layering (top to bottom): the base prompt — a user ``SYSTEM.md`` override
    (``paths.system_md_override()``) when present, else the packaged invariant core
    (``prompts/fragments/core.md``); then any ``AGENTS.md`` operating notes
    (``paths.agents_md_chain()``, global then project, Pi-style hierarchical merge);
    then the fragments for the disclosed connectors
    (``prompts/fragments/<connector_id>.md``), which get the last word.
    ...
    """
    override = system_md_override()
    parts = [_read(override) if override else _read(CORE_FRAGMENT)]
    parts.extend(_read(agents) for agents in agents_md_chain())
...
def save_system_prompt(content: str, custom_path: str | None = None) -> Path:
    """Write the prompt to the user ``SYSTEM.md``, or to ``custom_path`` for export.

    With no path it targets the writable override (``paths.system_md_target()``),
    ...
    """
    target = Path(custom_path) if custom_path else system_md_target()
    if target.resolve().is_relative_to(PACKAGE_DIR):
        raise ValueError(
            f"refusing to write into the packaged (read-only) prompt dir: {target}. "
            "Customize via SYSTEM.md (dacli prompt --edit) or DACLI.md instead."
        )
